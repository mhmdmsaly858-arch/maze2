...
from mazegen import MazeGenerator, EAST, SOUTH
...
def render_terminal(
    gen: MazeGenerator, show_path: bool = False, colour_idx: int = 0
) -> None:
    ...
    _ = set(gen.solution) if show_path else set()
    wall_col = WALL_COLOURS[colour_idx % len(WALL_COLOURS)]

    line = [
        [(wall_col + WALL_CH + RESET) for b in range(2 * w + 1)]
        ...
        for col in range(2 * w + 1):
            ...
            if on_h_edge and on_v_edge:
                ...
                if cell in gen.num_42:
                    line[row][col] = "\033[31m" + WALL_CH + RESET
                elif cell == gen.entry:
    ...
    if not show_path:
        ...
        for char in line:
            print("".join(char))
    if not len(gen.num_42):
        print("Error: Maze measurements are not enough to draw 42")
...
def build_generator(cfg: dict[str, str], seed: Optional[int]) -> MazeGenerator:
    ...
    gen = MazeGenerator(
        width=width, height=height, seed=seed, entry=entry, exit_=exit_
    )
...
def main() -> None:
    ...
    filse = ["Makefile", "a_maze_ing.py", "mazegen.py", config_path]

    try:
        cfg = parse_config(config_path)
        if cfg["OUTPUT_FILE"] in filse:
            raise ValueError("try another name for output file!")
        seed: Optional[int] = int(cfg["SEED"]) if "SEED" in cfg else None
        if int(cfg["WIDTH"]) < 3 or int(cfg["HEIGHT"]) < 3:
            raise ValueError("WIDTH and HEIGHT must both be >= 3")
        if int(cfg["WIDTH"]) > 120 or int(cfg["HEIGHT"]) > 120:
            raise ValueError("WIDTH and HEIGHT must both be <= 120")
